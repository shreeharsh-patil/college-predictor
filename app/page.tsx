  ...
  return <main id="main">
    <section className="hero-wash border-b border-slate-200/60 pb-12 pt-10 sm:pb-12">
      <div className="page-shell">
        ...
        <ExamCards />
        <div className="mt-7 flex flex-wrap justify-center gap-x-8 gap-y-3 text-xs text-slate-500"><span className="flex items-center gap-2"><BadgeCheck size={16} className="text-teal-600" /> No sign-up needed to explore</span><span className="flex items-center gap-2"><Fingerprint size={16} className="text-teal-600" /> Your details stay in your browser</span><span className="flex items-center gap-2"><ChartNoAxesCombined size={16} className="text-teal-600" /> 2025 Round 1–3 &amp; SVR cutoff data</span></div>
      </div>
    </section>
    <section id="how-it-works" className="page-shell py-16 sm:py-20">
      ...
      <div className="mt-11 grid gap-8 md:grid-cols-3 md:gap-12">
        {[{ icon: Fingerprint, title: "Enter your NEET profile", text: "Add your AIR, category, PwBD status, domicile, gender and college preference." }, { icon: ChartNoAxesCombined, title: "Compare published cutoffs", text: "Your AIR is compared directly with the selected round's opening and closing ranks." }, { icon: ListFilter, title: "Review sorted colleges", text: "Home-state and preferred college types appear first, followed by the closest rank fit." }].map(({ icon: Icon, title, text }, index) => <div key={title} className="relative"><div className="mb-5 flex items-center gap-4"><span className="flex size-12 items-center justify-center rounded-2xl border border-indigo-100 bg-indigo-50 text-indigo-600"><Icon size={23} strokeWidth={1.6} /></span><span className="text-xs font-medium text-slate-400">0{index + 1}</span>{index < 2 && <span className="hidden h-px flex-1 border-t border-dashed border-slate-200 md:block" />}</div><h3 className="text-[15px] font-semibold">{title}</h3><p className="mt-2 max-w-xs text-[13px] leading-6 text-slate-500">{text}</p></div>)}
      </div>
    </section>
    <section aria-labelledby="counselling-heading" className="page-shell pb-16 sm:pb-20">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-40px_#33415540] sm:p-9">
        ...
        <div className="mt-5 flex gap-3 rounded-2xl border border-amber-100 bg-amber-50 p-5"><ChartNoAxesCombined size={19} className="mt-0.5 shrink-0 text-amber-700" /><p className="text-xs leading-6 text-amber-900">Predictions use official 2025 AYUSH UG Round 1, Round 2, Round 3, and Stray Vacancy Round I and II allotment data. Final allotments depend on each year&apos;s official seat matrix, counselling rounds, eligibility rules, choice filling and category movement — use the results as a starting point, not a guarantee.</p></div>
      </div>
    </section>
